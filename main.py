...
class Data:
    def __init__(self) -> None:
        ...
        self.items, self.payments = group(read("olist_order_items_dataset.csv"), "order_id"), group(read("olist_order_payments_dataset.csv"), "order_id")
        ...
        self.trans = {val(r["product_category_name"]): val(r["product_category_name_english"]) for r in trans.to_dict("records")}
...
def compile_case(data: Data, case: dict[str, Any]) -> dict[str, Any]:
    ...
    order, items, payments = data.orders[oid], data.items.get(oid, []), data.payments.get(oid, [])
    ...
    cats = uniq([data.trans.get(val(data.products.get(pid, {}).get("product_category_name"))) or val(data.products.get(pid, {}).get("product_category_name")) for pid in product_ids], 5)
    item_ids = [f"{oid}:{int(x['order_item_id'])}" for x in items][:5]
    payment_ids = [f"{oid}:{int(x['payment_sequential'])}" for x in payments][:5]
    item_total = round(sum(num(x["price"]) for x in items), 2) if items else None
    freight = round(sum(num(x["freight_value"]) for x in items), 2) if items else None
    ...
    for seller in sellers:
        ...
        hv = variance(order.get("order_delivered_carrier_date"), limit)
        handoffs.append({"seller_id": seller, "shipping_limit_at": limit, "handoff_variance_hours": hv, "late_handoff": hv is not None and hv > 0})
    ...
    return {"case_id": case["case_id"], "case_assessment": {"primary_issue": primary, "secondary_issues": secondary, "case_status": "action_required" if refund > 0 else "no_action", "confidence": 1.0}, "affected_entities": {"order_ids": [oid], "item_ids": item_ids, "seller_ids": sellers, "payment_ids": payment_ids}, "customer_context": {"customer_unique_id": unique_id, "related_order_ids": related}, "product_context": {"product_ids": product_ids, "category_names": cats}, "delivery_analysis": {"delivered_at": val(order.get("order_delivered_customer_date")), "estimated_delivery_at": val(order.get("order_estimated_delivery_date")), "carrier_handoff_at": val(order.get("order_delivered_carrier_date")), "delivery_variance_hours": dv, "seller_handoff_analysis": handoffs, "late_handoff_seller_ids": late_sellers}, "payment_reconciliation": {"currency": "BRL", "item_total_brl": item_total, "freight_total_brl": freight, "expected_total_brl": expected, "payment_total_brl": paid, "difference_brl": diff, "reconciled": reconciled, "payment_types": uniq([val(x["payment_type"]) for x in payments], 5)}, "root_cause_analysis": {"ranked_causes": [{"cause_code": cause, "rank": 1}], "responsible_parties": [{"party_type": t, "party_id": x} for t, x in parties][:3]}, "evidence_ids": evidence[:20], "financial_resolution": {"currency": "BRL", "recommended_refund_brl": round(refund, 2)}, "resolution_actions": actions[:5]}
